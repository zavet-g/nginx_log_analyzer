...
"""
    ...
    async def test_monitor_log_file_empty_file(self, tmp_path):
        """Пустой файл не даёт записей, монитор продолжает ждать новых строк."""
        ...
        async def first_entry():
            async for entry in parser.monitor_log_file():
                return entry
            return None

        with pytest.raises(asyncio.TimeoutError):
            await asyncio.wait_for(first_entry(), timeout=2)
    ...
    async def test_monitor_log_file_nonexistent(self):
        ...
        async for _ in parser.monitor_log_file():
            raise AssertionError('Не должно быть записей для несуществующего файла')
    ...
    def test_log_pattern_matching(self, parser):
        ...
        test_cases = [
            ...
            {'line': 'invalid log line', 'should_match': False},
            {
